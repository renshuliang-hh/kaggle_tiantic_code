...
rf = RandomForestClassifier(n_estimators=230, warm_start=True, max_features=10, max_depth=9,
                            min_samples_split=9, min_samples_leaf=2, n_jobs=-1, verbose=0)
...
def get_top_n_features(titanic_train_data_X, titanic_train_data_Y, top_n_features):
    ...
    rf_param_grid = {'n_estimators': [230],'max_features':[10],'max_depth':[9],'criterion':['gini'],'min_samples_leaf':[9],'min_samples_split':[2]}
...
def clf(datas, label, pre_datas):
    ...
    xgc = XGBClassifier()
    xgc_param = {
            'n_estimators':[780], 'max_depth':[2],'learning_rate':[0.001],'subsample':[0.7,],'colsample_bytree':[0.5],'min_child_weight':[1],'gamma': [1]
...
if __name__ == '__main__':
    ...
    data_all = pd.read_csv('data/data_train_test_ 1.csv')
    ...
    StackingSubmission.to_csv('new_result/stakd_grid_all_10_fina_5.csv', index=False, sep=',')
