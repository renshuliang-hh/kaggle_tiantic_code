...
def clf(datas, label, pre_datas):
    ...
    et = ExtraTreesClassifier(n_estimators=370, n_jobs=-1, max_depth=8, min_samples_leaf=2, verbose=0)
    # 'max_depth': range(2, 15, 1) ,'max_features':[10],'max_depth':[9],'criterion':['gini'],'min_samples_leaf':[9],'min_samples_split':[2]


    xgc = XGBClassifier()
    param_grid = {'n_estimators': [40],'max_depth':[8],'min_samples_leaf':[6]}


    gc = GridSearchCV(rfc , param_grid=param_grid, cv=10,scoring='roc_auc')
