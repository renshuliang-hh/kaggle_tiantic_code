...
def clf(datas, label, pre_datas):
    ...
    gb_est = GradientBoostingClassifier(random_state=0)
    # 'max_depth': range(2, 15, 1),'learning_rate':[0.1],'max_features':[13],'max_depth':[2],'min_samples_leaf':[3],'min_samples_split':[2]


    xgc = XGBClassifier()
    param_grid = {'n_estimators': [50],'max_depth':[2],'learning_rate':[0.3],'min_samples_leaf':[2]}


    gc = GridSearchCV(gb_est , param_grid=param_grid, cv=5)
