...
rf = RandomForestClassifier(n_estimators=270, warm_start=True, max_depth=9,
                            min_samples_split=2, min_samples_leaf=2, n_jobs=-1, verbose=0)

ada = AdaBoostClassifier(n_estimators=100, learning_rate=0.05)

et = ExtraTreesClassifier(n_estimators=630, n_jobs=-1, max_depth=9,min_samples_leaf=1, verbose=0)

gb = GradientBoostingClassifier(n_estimators=50, learning_rate=0.3, min_samples_split=2, min_samples_leaf=2,
...
def get_top_n_features(titanic_train_data_X, titanic_train_data_Y, top_n_features):
    ...
    rf_param_grid = {'n_estimators': [270],'max_depth': [9],'min_samples_leaf':[2]}
    ...
    ada_param_grid = {'n_estimators': [100],'learning_rate':[0.05]}
    ...
    et_param_grid = {'n_estimators': [630],'max_depth':[9],'min_samples_leaf':[1]}
    ...
    gb_param_grid = {'n_estimators': [50],'max_depth':[2],'learning_rate':[0.3],'min_samples_leaf':[2]}
    ...
    features_top_n = pd.concat(
        [features_top_n_rf, features_top_n_ada, features_top_n_et, features_top_n_gb],
...
def clf(datas, label, pre_datas):
    ...
    x_train = np.concatenate((rf_oof_train, ada_oof_train, et_oof_train, gb_oof_train, svm_oof_train),
                             axis=1)
    x_test = np.concatenate((rf_oof_test, ada_oof_test, et_oof_test, gb_oof_test,  svm_oof_test), axis=1)
    x_pre = np.concatenate((rf_oof_pre, ada_oof_pre, et_oof_pre, gb_oof_pre,svm_oof_pre), axis=1)

    xgc = XGBClassifier()
    xgc_param = {
        'n_estimators': [50],'max_depth':[1],'learning_rate':[0.001]
        # 'learning_rate':np.linspace(0.01,2,20),'n_estimators':[780], 'max_depth':[2],'learning_rate':[0.001],'subsample':[0.7,],'colsample_bytree':[0.5],'min_child_weight':[1],'gamma': [1]
...
if __name__ == '__main__':
    train_df = pd.read_csv("new_data/0.79_train.csv")
    test_df = pd.read_csv("new_data/0.79_test.csv")
    X_train = train_df.drop('Survived', axis=1)
    Y_train = train_df['Survived']
    PassengerId = test_df['PassengerId']
    X_test = test_df.drop('PassengerId', axis=1).copy()
    pre_y = clf(X_train, Y_train, X_test)

    StackingSubmission = pd.DataFrame({'PassengerId': PassengerId, 'Survived': pre_y})
    StackingSubmission.to_csv('new_result/stakd_grid_all_0.79.csv', index=False, sep=',')
