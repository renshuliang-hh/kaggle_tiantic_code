...
def clf(datas, label, pre_datas):
    ...
    # 'max_depth': range(2, 15, 1),'max_features':[8],'max_depth':[3],'criterion':['gini'],'min_samples_leaf':[1],'min_samples_split':[2]


    xgc = XGBClassifier()
    param_grid = {'n_estimators': [630],'max_depth':[9],'min_samples_leaf':[1]}


    gc = GridSearchCV(et , param_grid=param_grid, cv=5)
